...
export default function AnalyticsDashboard() {
  ...
  if (authLoading) return <div className="p-20 text-center uppercase editorial-label text-[var(--color-text-tertiary)]">Loading...</div>;

  if (profile?.role !== 'admin') {
    return (
      <div className="text-center py-20">
        <p className="text-[var(--color-text-secondary)]">Access denied. Admin only.</p>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        <select
          ...
          className="px-4 py-2 border border-[var(--color-border-primary)] rounded-lg editorial-label bg-[var(--color-bg-secondary)]"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          ...
          <span className="editorial-label text-[var(--color-text-tertiary)]">Page Views</span>
        ...
        >
          ...
          <span className="editorial-label text-[var(--color-text-tertiary)]">Unique Visitors</span>
        ...
        >
          ...
          <span className="editorial-label text-[var(--color-text-tertiary)]">Avg. Session</span>
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <MousePointer className="w-5 h-5 text-[var(--color-text-tertiary)]" />
            <span className="flex items-center gap-1 text-red-500 text-sm font-bold">
              <TrendingDown className="w-3 h-3" /> -2%
            </span>
          </div>
          <span className="editorial-label text-[var(--color-text-tertiary)]">Bounce Rate</span>
            ...
            {data.topPages.map((page, idx) => (
              <div key={idx} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-mono text-[var(--color-text-secondary)] w-16 text-right">{page.views.toLocaleString()}</span>
                </div>
              </div>
            ))}
          </div>
            ...
            {data.trafficSources.map((source, idx) => (
              <div key={idx} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-mono text-[var(--color-text-secondary)] w-16 text-right">{source.visitors.toLocaleString()}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="bg-[var(--color-card-bg)] p-6 border border-[var(--color-border-primary)]">
        ...
        <div className="space-y-3">
          {data.topArticles.map((article, idx) => (
            <div key={idx} className="flex items-center justify-between py-3 border-b border-[var(--color-border-primary)] last:border-0">
              <div className="flex items-center gap-4">
                <span className="w-8 h-8 bg-[var(--color-bg-tertiary)] rounded-full flex items-center justify-center font-bold text-[var(--color-text-tertiary)]">
                  {idx + 1}
                </span>
